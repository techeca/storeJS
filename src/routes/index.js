...
router.get('/categorias', (req, res) => {
  //if(isNaN(req.params) & !isNaN(req.params)){console.log('true?')};
      ...
      pool.execute('SELECT * FROM category', (err, rows, fields) => {
      if(err) err;
...
router.get('/productos/:name?', (req, res) => {
    ...
    pool.execute('SELECT * FROM product WHERE name LIKE ?', [`%${name}%`], (err, rows, fields) => {
     if(err) throw err;
...
router.get('/productosByCategoria?', async (req, res, next) => {
  ...
  try {
    ...
    if(isNaN(page) || page < 0) throw `Parametro 'page' incorrecto... ej: /?id=1&page=1`;
      ...
      let [rows, fields] = await promisePool.execute(`SELECT COUNT(*) as totalProductos FROM product WHERE category = ?`, [id]);
      totalProductos = rows;
        pool.execute(`SELECT * FROM product WHERE category = ? LIMIT ${limite} OFFSET ${page * limite}`, [id], (err, rows, fields) => {
          if(err) throw err;
